...
| Suite | Description |
|-------|-------------|
| `test_http_parser` | Request line parsing, header parsing, methods, versions, absolute URIs, error handling, edge cases |
| `test_router` | Route registration, lookup, method matching, 404/405/501 fallbacks, boundary conditions |
| `test_http_request` | Header lookup, string view handling, missing headers |
| `test_http_response` | Response serialization, status codes, Content-Length calculation, body handling |

Tests marked with `TEST_IGNORE_MESSAGE("TODO: ...")` are TDD placeholders for features not yet implemented.
...
- **No custom response headers**: `response.headers[]` are not serialized yet
- **No query string stripping**: `/search?q=test` won't match route `/search`
